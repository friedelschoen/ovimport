...
class ServiceTabler extends CommonTabler<ServiceFrame> {
    private Table routePoints(ServiceFrame frame, String dataset) {
    ...
    private Table destinationDisplays(ServiceFrame frame, String dataset) {
        Stream<String[]> rows = Stream.empty();
    ...
    private Table destinationDisplayVariants(ServiceFrame frame, String dataset) {
        Stream<String[]> rows = Stream.empty();
    ...
    private Table scheduledStopPointTariffzones(ServiceFrame frame, String dataset) {
        Stream<String[]> rows = Stream.empty();
            ...
            rows = frame.getScheduledStopPoints().getScheduledStopPoint().parallelStream()
                    .flatMap(pnt -> pnt.getTariffZones() != null
                            ? pnt.getTariffZones().getTariffZoneRef().stream()
                                    .map(t -> new String[] { dataset, pnt.getId(), str(pnt.getVersion(), () -> "any"),
                                            refString(t) })
                            : Stream.empty());
        }
    ...
    private String[] stoppointInJourneyPattern(String dataset, String jpID, String jpVersion,
    ...
    private String[] timingpointInJourneyPattern(String dataset, String jpID, String jpVersion,
            String name, String routeRef,
            String dirType,
    ...
    private Table journeyPatterns(ServiceFrame frame, String dataset) {
        Stream<String[]> rows = Stream.empty();
                        ...
                        return sjp.getPointsInSequence()
                                ...
                                .map(p -> switch (p) {
                                    case StopPointInJourneyPattern sp ->
                                        stoppointInJourneyPattern(dataset, sjp.getId(),
                                    ...
                                    case TimingPointInJourneyPattern tp ->
                                        timingpointInJourneyPattern(dataset, sjp.getId(),
                                                str(sjp.getVersion(), () -> "any"),
                                                text(sjp.getName()),
                                                refString(sjp.getRouteRef()),
                                                val(sjp.getDirectionType()),
            ...
            Stream<String[]> deadRunJourneys = frame.getJourneyPatterns().getDeadRunJourneyPattern().parallelStream()
                    .flatMap(drp -> {
                        ...
                        return drp.getPointsInSequence()
                                ...
                                .stream()
                                .map(p -> switch (p) {
                                    case StopPointInJourneyPattern sp -> stoppointInJourneyPattern(dataset, drp.getId(),
                                            str(drp.getVersion(), () -> "any"),
                                            text(drp.getName()),
                                            null, null, null,
                                            sp);
                                    case TimingPointInJourneyPattern tp ->
                                        timingpointInJourneyPattern(dataset, drp.getId(),
                                                str(drp.getVersion(), () -> "any"),
                                                text(drp.getName()),
                                                null, null, null,
                                                tp);
    ...
    private Table timeDemandTypes(ServiceFrame frame, String dataset) {
        Stream<String[]> rows = Stream.empty();
    ...
    private Table notices(ServiceFrame frame, String dataset) {
        Stream<String[]> rows = Stream.empty();
    ...
    @Override
    public Stream<Table> apply(ServiceFrame frame, String dataset) {
        return Stream.of(
                routePoints(frame, dataset),
                routeLinks(frame, dataset),
                routes(frame, dataset),
                lines(frame, dataset),
                destinationDisplays(frame, dataset),
                destinationDisplayVariants(frame, dataset),
                scheduledStopPoints(frame, dataset),
                scheduledStopPointTariffzones(frame, dataset),
                stopAssignments(frame, dataset),
                timingPoints(frame, dataset),
                timingLinks(frame, dataset),
                journeyPatterns(frame, dataset),
                timeDemandTypes(frame, dataset),
                notices(frame, dataset),
                noticeAssignments(frame, dataset),
                stopAreas(frame, dataset));
    }
}
